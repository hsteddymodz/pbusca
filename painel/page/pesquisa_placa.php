...
<script>
    ...
    function processar_dados(userData) {
		...
		$.get('https://probusca.com/painel/page/pesquisa_placa.html').done(function(htmlContent) {
			let templateScript = Handlebars.compile(htmlContent);
			let dataTemplate = {
				dadospessoais: {
					nome: userData.cadastral.nome,
					mae: userData.cadastral.mae,
					rg: userData.cadastral.rg || 'Indisponível',
			...
			dataTemplate.carro = userData.carro;

			dataTemplate.carro.isRoubado = dataTemplate.carro['roubado?'];
			if(dataTemplate.carro.isRoubado)
				dataTemplate.carro.roubado = 'Sim - ' + dataAtualizacaoRouboFurto;
			else
				dataTemplate.carro.roubado = 'Não';

			dataTemplate.carro.anoExe = dataTemplate.carro.anoExe || 'Indisponível';
			dataTemplate.carro.ipva = dataTemplate.carro.ipva || 'Indisponível';

			if(userData.enderecos && userData.enderecos.length > 0) {
				let cep = String(userData.enderecos[0].cep);
				dataTemplate.endereco = {
					endereco: userData.enderecos[0].endereco,
					numero: userData.enderecos[0].numero || 'Indisponível',
					complemento: userData.enderecos[0].compl || 'Indisponível',
					bairro: userData.enderecos[0].bairro || 'Indisponível',
					cidade: userData.enderecos[0].cidade || 'Indisponível',
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
	<div class="row">
		<ol class="breadcrumb">
			...
			<li class="active">Pesquisa Placa</li>
		</ol>
	</div><!--/.row-->

	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">Pesquisa Placa</h1>
		</div>
	</div><!--/.row-->
					...
					<form action="" id="form_basica" method="post">
						<div class="col-lg-2 col-sm-4 col-md-3">
							...
							<div class="form-group">
								...
								<input type="text" name="dado" required class="form-control" placeholder="Digite a Placa">
							</div>
